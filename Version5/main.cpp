...

{
   ...
   for (int current_time = 0; current_time < end_time; current_time++) { //  loop over time intervals
      ...
      case land:
         ...
         break;
      case takeoff:
         plane_arrivals.fly(current_time);
         break;
      case idle:
         runway_arrivals.run_idle(1, current_time);
      }
...
//Phase 5
int main5()     //  Airport simulation program
/*
Pre:  The user must supply the number of time intervals the simulation is to
      run, the expected number of planes arriving, the expected number
      of planes departing per time interval, and the
...

{
   ...
   Random variable;
    Random var1;
   Runway small_airport(queue_limit);
   Plane moving_plane;  //declaring earlier for urgent landing;
   for (int current_time = 0; current_time < end_time; current_time++) { //  loop over time intervals
      int number_arrivals = variable.poisson(arrival_rate);  //  current arrival requests
      bool urgent_landing = false;
      for (int i = 0; i < number_arrivals; i++) {
         
          int fuel_amount = var1.random_integer(0, 5); //  fuel level from 0..5
          Plane current_plane(flight_number++, current_time, arriving, fuel_amount);
          //if out of fuel then the place lands immediately, BUT we can't land 2 planes without fuel at the same!!!!
          if (current_plane.getState() == arriving_out_of_fuel && !urgent_landing)
          {
              small_airport.can_land(current_plane, true);
      ...
      if (urgent_landing) moving_plane.land(current_time);
    ...
    small_airport.shut_down(end_time);
    cout << "Number of planes crashed: " << Plane::planes_crashed << endl;
    return 0;
}
